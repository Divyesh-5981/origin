...
function buildParagraphs(analysis: StoryAnalysis): string[] {
  ...
  paragraphs.push(
    `${opening} ${name}'s story with ${passion} didn't begin with fanfare or recognition. ${originText} ${origin.sentences.length > 1 ? origin.sentences.slice(1).join(" ") : "What started as a flicker of interest would, over time, grow into something undeniable — a force that would shape not just a career but an identity."}`,
  );
  ...
  const deepeningSentences = [
    ...
    `The relationship with ${passion} was raw, unformed, full of trial and error.`,
    ...
    `The craft of ${passion} demanded patience ${name} didn't always have, and taught lessons ${name} didn't always want to learn.`,
  ];
  ...
  const struggleSentences = [
    ...
    lowest.emotionalTone === "struggling"
      ? `These were the days when ${passion} felt less like a calling and more like a burden, when the gap between where ${name} was and where they wanted to be seemed unbridgeable.`
  ...
  const growthSentences = [
    ...
    `${passion} had transformed from an interest into a practice, and from a practice into a craft.`,
  ...
  paragraphs.push(
    `${reflection} ${name}'s journey with ${passion} is not a tale of overnight success or innate genius. It is, at its core, the story of ${oneSentence}. The origin was never about talent finding its stage — it was about persistence finding its voice.`,
  );
  ...
  paragraphs.push(
    `Today, ${name} stands at a vantage point that the beginning could not have imagined. ${dreamText} The passion that started as a spark has become a guiding light, and the journey that tested everything has become the foundation of everything that comes next.`,
  );

  // Paragraph 8: Closing
  const closing = pick(CLOSING_SENTENCES, seed + 2);
  paragraphs.push(
    `${closing} ${name}'s origin story with ${passion} is a reminder that every expert was once a beginner, every success was once a maybe, and every legend was once someone who simply refused to stop.`,
  );
...
function buildOriginStory(analysis: StoryAnalysis): string {
  ...
  // Ensure we hit the 1000-1500 word range
  const wordCount = story.trim().split(/\s+/).filter(Boolean).length;
  if (wordCount < 1000) {
    // Add reflective expansion paragraphs
    const { name, passion, profession } = analysis;
    const expansions = [
      `The pursuit of ${passion}, ${name} would come to understand, is not a linear path but a spiral. Each pass brings you back to familiar territory, but at a higher elevation, seeing what you couldn't see before.`,
...
function inferMission(analysis: StoryAnalysis): string {
  ...
  return `To channel ${passion} into work that matters — and to ${dreamText}.`;
}
...
function inferMotivation(analysis: StoryAnalysis): string {
  ...
  return `The conviction that ${dreamText} is not just possible but necessary, and that ${passion} is the vehicle to get there.`;
}
  ...
  return [
    {
      ...
      body: origin.raw.length > 0
        ? origin.raw
        : `Where ${name}'s passion for ${passion} first took root — a moment of curiosity that would refuse to fade.`,
    },
    {
      key: "failure",
      title: "The Trial",
      body: lowest.raw.length > 0
        ? lowest.raw
        : `The low point that tested whether ${passion} was a hobby or a calling.`,
    },
    {
      key: "breakthrough",
...
function buildSocialAssets(analysis: StoryAnalysis): Story["social"] {
  ...
  return {
    linkedin: `My origin story: how a passion for ${passion} shaped my path as a ${profession} — from the first spark, through the lowest point, to the dream of ${dream.raw.length > 0 ? dream.raw.toLowerCase().replace(/[.!?]$/, "") : "building something that lasts"}.`,
    twitterThread: [
      `1/ Every passion has an origin. Here's mine — built around ${passion}. 🧵`,
    ...
    instagram: `Every legend begins somewhere. This is where mine began. ✨ #${passion.replace(/\s+/g, "")} #OriginStory`,
    portfolioBio: `${name} is a ${profession} driven by a lifelong passion for ${passion}, working toward ${dream.raw.length > 0 ? dream.raw.toLowerCase().replace(/[.!?]$/, "") : "a future worth building"}.`,
    resumeSummary: `${profession} with a proven record of turning curiosity about ${passion} into lasting results. Resilient, adaptable, and driven by a vision that has been tested and refined through real-world challenge.`,
  };
}
...
function buildPoster(analysis: StoryAnalysis): Story["poster"] {
  ...
  return {
    ...
    title: analysis.name,
    subtitle: `A ${analysis.passion} Origin Story`,
...
function analyzeAnswers(answers: Answers): StoryAnalysis {
  ...
  const analysis: StoryAnalysis = {
    ...
    dominantTheme: "discovery", // placeholder, set below
    emotionalArc,
...
/**
 * Generates a complete, high-quality Story object using heuristic analysis
 * of the user's answers. No LLM API calls — runs instantly.
 *
 * The algorithm:
 * 1. Analyzes each narrative answer for sentences, keywords, and emotional tone
 * 2. Detects the dominant narrative theme (underdog, discovery, transformation, etc.)
 * 3. Composes a 1000-1500 word cinematic origin story using theme-appropriate
 *    openings, transitions, and closings, weaving in the user's actual words
 * 4. Infers character strengths/weaknesses/values from emotional patterns
 * 5. Generates timeline, quote, trailer script, social assets, and poster
 *    all personalized to the detected theme and emotional arc
 */
