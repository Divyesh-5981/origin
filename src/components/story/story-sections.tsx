...
function FilmFrame({
	...
	// Generate sprockets for realistic film edge
	...
	return (
		<motion.div
			...
			transition={{ delay: index * 0.15 + 0.1, duration: 0.7, ease: "easeOut" }}
			className={cn(
				"group flex-shrink-0 w-[280px] sm:w-[320px] flex flex-col relative transform-gpu hover:z-10",
				isFirst && "rounded-l-lg",
				isLast && "rounded-r-lg",
			)}
		>
			{/* Film Base (Continuous Black Celluloid) */}
			<div
				className={cn(
					"absolute inset-0 bg-[#0a0a0a] shadow-[0_0_30px_rgba(0,0,0,0.9)] border-y border-white/5",
					isFirst && "border-l rounded-l-lg",
					isLast && "border-r rounded-r-lg",
				)}
			/>

			{/* Frame separator lines on celluloid (only between frames) */}
			{!isLast && (
				<div className="absolute right-0 top-0 bottom-0 w-[1px] bg-white/5 z-20" />
			)}

			{/* Top Sprocket Track */}
			<div className="relative h-10 border-b border-white/5 flex items-center justify-between px-2 bg-gradient-to-b from-[#151515] to-[#050505] overflow-hidden">
				{sprockets.map((_, i) => (
					<div
						key={i}
						className="w-2 h-3.5 bg-black rounded-sm shadow-[inset_0_2px_6px_rgba(0,0,0,1),_0_1px_0_rgba(255,255,255,0.08)]"
					/>
				))}
				{/* Edge markings */}
				<div className="absolute top-1 left-3 text-[6px] text-white/30 font-mono tracking-widest z-10 hidden sm:block">
					{index + 1}K • {5200 + index * 12}
				</div>
			</div>

			{/* Frame Visual / Content Area */}
			<div className="px-4 py-6 relative h-[250px] flex flex-col group overflow-hidden">
				{/* Internal illuminated screen panel effect */}
				<div className="absolute inset-x-3 inset-y-2 bg-gradient-to-b from-white/[0.03] to-transparent border border-white/10 rounded group-hover:border-white/30 group-hover:bg-white/[0.05] transition-all duration-500 pointer-events-none" />

				{/* Cinematic ambient glow that reveals on hover */}
				<div className="absolute inset-0 bg-ignition-orange/0 group-hover:bg-ignition-orange/10 blur-2xl transition-colors duration-700 pointer-events-none" />

				<div className="relative z-10 flex-1 flex flex-col justify-center px-4">
					<div className="flex items-center gap-2 mb-4">
						<span className="flex items-center justify-center w-5 h-5 rounded-[2px] bg-white/10 text-white text-[9px] font-bold font-mono">
							{index + 1}
						</span>
						<span className="text-[9px] font-mono tracking-[0.25em] text-ignition-orange font-bold uppercase">
							ACT 0{index + 1}
						</span>
						<div className="flex-1 h-[1px] bg-gradient-to-r from-ignition-orange/50 to-transparent" />
					</div>

					<h4 className="font-heading text-lg sm:text-xl font-bold text-white tracking-tight mb-2 group-hover:text-glow transition-all duration-300">
						{stage.title}
					</h4>

					<p className="text-xs leading-relaxed text-white/70 line-clamp-4 font-serif italic selection:bg-ignition-orange/30">
						{stage.body}
					</p>
				</div>

				{/* Camera Focus/Corner guides */}
				<div className="absolute top-4 left-4 w-2 h-2 border-t border-l border-white/20 transition-all duration-300 group-hover:w-3 group-hover:h-3 group-hover:border-electric-cyan" />
				<div className="absolute top-4 right-4 w-2 h-2 border-t border-r border-white/20 transition-all duration-300 group-hover:w-3 group-hover:h-3 group-hover:border-electric-cyan" />
				<div className="absolute bottom-4 left-4 w-2 h-2 border-b border-l border-white/20 transition-all duration-300 group-hover:w-3 group-hover:h-3 group-hover:border-electric-cyan" />
				<div className="absolute bottom-4 right-4 w-2 h-2 border-b border-r border-white/20 transition-all duration-300 group-hover:w-3 group-hover:h-3 group-hover:border-electric-cyan" />
			</div>

			{/* Bottom Sprocket Track */}
			<div className="relative h-10 border-t border-white/5 flex items-center justify-between px-2 bg-gradient-to-t from-[#151515] to-[#050505] overflow-hidden">
				{sprockets.map((_, i) => (
					<div
						key={i}
						className="w-2 h-3.5 bg-black rounded-sm shadow-[inset_0_-2px_6px_rgba(0,0,0,1),_0_-1px_0_rgba(255,255,255,0.08)]"
					/>
				))}
				{/* Analog optical audio track representation */}
				<div className="absolute inset-y-1 left-3 w-1.5 opacity-30 flex gap-[1px] overflow-hidden">
					<div className="w-[1px] h-[90%] my-auto bg-electric-cyan animate-pulse" />
					<div className="w-[1px] h-full bg-electric-cyan opacity-40 mix-blend-screen" />
					<div className="w-[1px] h-[70%] my-auto bg-electric-cyan" />
				</div>
			</div>
		</motion.div>
	);
					...
					<nav className="flex flex-col gap-2" aria-label="Scene chapters">
						{(
							[
								...
								{
									id: "timeline",
									label: "Scene II: Chronology",
									icon: Calendar,
								},
								{
									id: "character",
									label: "Scene III: Protagonist",
									icon: User,
								},
							...
							return (
								...
								>
									<Icon
										className={cn(
											"size-4 shrink-0",
											active ? "text-ignition-orange" : "text-muted-foreground",
										)}
									/>
									{tab.label}
								</button>
							...
							{activeTab === "timeline" && (
								<div className="flex flex-col gap-6">
									<div className="border-b border-white/10 pb-4 flex items-center justify-between">
										<h3 className="font-heading text-2xl font-bold tracking-tight text-white flex items-center gap-2">
											<Film className="size-5 text-ignition-orange animate-pulse" />
											Chronology Storyboard
										</h3>
									</div>
									{/* Ultra-realistic Continuous Film Strip Timeline */}
									<div className="w-full relative py-6">
										{/* Shadow masking for edges */}
										<div className="absolute inset-y-0 left-0 w-8 bg-gradient-to-r from-background to-transparent z-20 pointer-events-none" />
										<div className="absolute inset-y-0 right-0 w-8 bg-gradient-to-l from-background to-transparent z-20 pointer-events-none" />

										<div className="w-full overflow-x-auto pb-4 pt-2 -mt-2 -mb-4 flex scrollbar-thin scrollbar-thumb-muted items-center bg-black/20 rounded-xl shadow-inner">
											<div className="flex flex-nowrap drop-shadow-[0_20px_20px_rgba(0,0,0,0.8)] px-4 mx-auto">
								...
								<div className="flex flex-col gap-6">
									...
									<GlassCard className="p-6 sm:p-8">
										<div className="relative z-10">
											<span
												className="font-heading text-4xl text-ignition-orange/20 block -mb-2"
												aria-hidden
											>
												&ldquo;
											</span>
									...
									<div className="flex flex-col sm:flex-row gap-4 items-center justify-between p-6 bg-white/5 rounded-xl border border-white/10 backdrop-blur-md">
										<div className="text-left">
											<p className="text-sm font-semibold text-white">
												Public Share Page Ready
											</p>
											<p className="text-xs text-muted-foreground">
												Open or copy the public link to showcase your film
												origin story.
											</p>
										</div>
										<Link
